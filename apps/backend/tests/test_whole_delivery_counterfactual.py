...
class Baseline:
    ...
    The incident window opens here, which is the whole of the attribution rule: Lena's amendment
    is already in both systems by this point, so no effect of hers can be counted against a case
...
async def census(intake: Intake, *, case_id: UUID, since: Baseline) -> Effects:
    ...
    Each of the manifest's five kinds is counted from a durable condition rather than from an
    intention: an amendment the order system accepted, a message the provider took, a set of
    reservations that is no longer the set this window opened with, a production task this case
    is holding, and a track that ended on the owner's desk. Anything outside the window -- the
    customer's own amendment above all -- is not this case's and is not counted.
