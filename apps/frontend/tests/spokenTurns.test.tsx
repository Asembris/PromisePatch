...
import {
  ...
  TURN_DID_NOT_HAPPEN,
} from "../src/features/voice/turnVoice";
...
function accepted(speech: string, spoken: string = speech): unknown {
  return {
    case_id: CASE_ID,
...
describe("the backend’s sentence, spoken on turn resolution", () => {
  it("reads the accepted turn’s speech aloud without anybody pressing anything", async () => {
    const REPLY =
      "The raspberries are the missing ones. Working out what that touches now.";
  ...
  it("speaks it byte for byte, adding and removing nothing", async () => {
    const REPLY =
      "Planned, and waiting for you. Nothing has been done yet. Say yes to go ahead.";
    const { stream } = mountAtCase(CLARIFYING_CASE, {
      [CLARIFY_PATH]: () => json(accepted(REPLY), 202),
    });
    ...
    const spoken = voice.spoken.find((utterance) =>
      utterance.text.includes("Planned"),
    );
  ...
  it("speaks a confirmation’s answer too, and it is the backend’s", async () => {
    const REPLY =
      "Going ahead with that plan now. Nothing is finished until each order says so.";
  ...
  it("reads the short rendering, never the long one, when the two differ", async () => {
    // The one place the two renderings are told apart. `speech` is what the panel shows; the
    // loudspeaker gets `spoken`, which is the same case the backend composed shorter for a
    // listener (ADR-0014). Neither is made here, and the long one must never be heard.
    const SHOWN =
      "Planned, and waiting for you. Nothing has been done yet. Three orders are affected, two were left alone, and each of them has a reason on the screen beside it.";
    const HEARD = "Planned, and waiting for you. Nothing has been done yet.";
    const { stream } = mountAtCase(CLARIFYING_CASE, {
      [CLARIFY_PATH]: () => json(accepted(SHOWN, HEARD), 202),
    });
    ...
    await waitFor(() => {
      expect(voice.said).toContain(HEARD);
    });
    expect(voice.said).not.toContain(SHOWN);
    // And the long one is on the screen, unchanged, because nothing was removed from it.
  ...
  it("says nothing at all on arrival, because nobody took a turn", async () => {
    ...
    expect(screen.getByTestId("conversation-speech")).toHaveTextContent(
      /Planned, and waiting/,
    );
...
describe("the acknowledgement while a turn is in flight", () => {
  ...
  it("claims no outcome, names no case and holds no digit", () => {
    ...
    for (const word of CLAIM_WORDS) {
      expect(TURN_ACKNOWLEDGEMENT.toLowerCase()).not.toMatch(
        new RegExp(`\\b${word}\\b`),
      );
    }
...
describe("a refused turn", () => {
    ...
    const { stream } = mountAtCase(PLANNED_CASE, {
      [CONFIRM_PATH]: () =>
        apiError(
          409,
          "CASE_MOVED_ON",
          "that plan is not the one on offer any more",
        ),
    });
  ...
  it("claims no outcome and holds no digit either", () => {
    ...
    for (const word of CLAIM_WORDS) {
      expect(TURN_DID_NOT_HAPPEN.toLowerCase()).not.toMatch(
        new RegExp(`\\b${word}\\b`),
      );
    }
...
describe("the manual control, now that every turn speaks for itself", () => {
  it("replays the standing status without taking a turn", async () => {
    ...
    expect(voice.said).toEqual([PLANNED_CASE.spoken]);
