...
| 5 | **The Telegram customer channel is UNBUILT and DEFERRED to the G8 work that needs it** — the five complete deployed rehearsals from clean fixtures, each of which the roadmap requires to run with *real customer transport*. It is deferred to a gate rather than to a new phase because the roadmap forbids one: work handled within existing gates does not create a phase, and the "P6.3" the earlier records cited here name is not a phase that exists. It is a **G6** requirement — ADR-0006 chose it and nothing implements it: no bot, no webhook ingress, no `secret_token` check, no `update_id` deduplication, no send path, and no `pp channel check`. The consent protocol itself is real and durable; what is missing is the transport between it and a real person's phone. It is **recorded rather than deleted** | deferral of a G6 requirement, target gate named | [`prerequisites-integration-cost-and-limitations.md`](prerequisites-integration-cost-and-limitations.md) §4.1, [`p6.2-first-deployment.md`](p6.2-first-deployment.md) §9 |
...
- **The Telegram customer channel**, which is G6's and is deferred into G8's five deployed
  rehearsals, the gate that requires real customer transport.
